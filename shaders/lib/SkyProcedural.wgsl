...
fn evaluateAtmosphericSkyFast(viewDir: vec3<f32>, lightDir: vec3<f32>, lightColor: vec3<f32>, isNight: f32, animTime: f32) -> vec3<f32> {
    ...
    if (isNight < 0.5) {
        if (sunCos < 0.25) {
            let t = clamp(sunCos / 0.25, 0.0, 1.0);
            skyZenith = mix(vec3<f32>(0.12, 0.08, 0.32), vec3<f32>(0.08, 0.35, 0.85), t);
            skyHorizon = mix(vec3<f32>(1.0, 0.32, 0.10), vec3<f32>(0.45, 0.75, 0.98), t);
        } else {
            skyZenith = vec3<f32>(0.08, 0.35, 0.85);
            skyHorizon = vec3<f32>(0.45, 0.75, 0.98);
        }
    } else {
        skyZenith = vec3<f32>(0.02, 0.04, 0.14);
        skyHorizon = vec3<f32>(0.08, 0.14, 0.35);
    }
...
fn evaluateUltraStylizedSky(viewDir: vec3<f32>, lightDir: vec3<f32>, lightColor: vec3<f32>, isNight: f32, animTime: f32) -> vec3<f32> {
    ...
    if (isNight < 0.5) {
        if (sunCosZenith < 0.3) {
            let t = clamp(sunCosZenith / 0.3, 0.0, 1.0);
            skyZenith  = mix(vec3<f32>(0.16, 0.06, 0.35), vec3<f32>(0.06, 0.32, 0.82), t);
            skyHorizon = mix(vec3<f32>(1.0, 0.28, 0.08), vec3<f32>(0.42, 0.75, 0.98), t);
        } else {
            let t = clamp((sunCosZenith - 0.3) / 0.7, 0.0, 1.0);
            skyZenith  = mix(vec3<f32>(0.06, 0.32, 0.82), vec3<f32>(0.04, 0.26, 0.75), t);
            skyHorizon = mix(vec3<f32>(0.42, 0.75, 0.98), vec3<f32>(0.60, 0.85, 1.0), t);
        }
    } else {
        let t = clamp(sunCosZenith, 0.0, 1.0);
        skyZenith  = mix(vec3<f32>(0.015, 0.03, 0.12), vec3<f32>(0.01, 0.02, 0.08), t);
        skyHorizon = mix(vec3<f32>(0.06, 0.12, 0.32), vec3<f32>(0.10, 0.18, 0.42), t);
    }
    ...
    if (dir.y > 0.02) {
        ...
        if (cDensity > 0.01) {
            ...
            if (isNight < 0.5) {
                let sunTint = mix(vec3<f32>(1.0, 0.45, 0.2), vec3<f32>(1.0, 0.98, 0.90), clamp(sunCosZenith * 2.5, 0.0, 1.0));
                cLit = mix(vec3<f32>(0.95, 0.95, 1.0), sunTint, 0.5) * (1.0 + rimLight);
                cShadow = mix(vec3<f32>(0.2, 0.25, 0.45), vec3<f32>(0.5, 0.2, 0.3), clamp(1.0 - sunCosZenith * 3.0, 0.0, 1.0));
            } else {
